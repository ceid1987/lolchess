...
class TakenPiecesPanel extends JPanel {
    ...
    public void redo(final MoveLog moveLog) {
        ...
        final List<Piece> p2TakenPieces = new ArrayList<>();
        final List<Piece> p1TakenPieces = new ArrayList<>();

        for(final Move move : moveLog.getMoves()) {
            if(move.isAttack()) {
                final Piece takenPiece = move.getAttackedPiece();
                if(takenPiece.getPieceAlliance().isP2()) {
                    p2TakenPieces.add(takenPiece);
                } else if(takenPiece.getPieceAlliance().isP1()){
                    p1TakenPieces.add(takenPiece);
                } else {
        ...
        Collections.sort(p2TakenPieces, new Comparator<Piece>() {
            @Override
        ...
        Collections.sort(p1TakenPieces, new Comparator<Piece>() {
            @Override
            public int compare(final Piece p1, final Piece p2) {
                return Ints.compare(p1.getPieceValue(), p2.getPieceValue());
            }
        ...
        for (final Piece takenPiece : p2TakenPieces) {
            try {
                final BufferedImage image = ImageIO.read(new File("imgs/"
        ...
        for (final Piece takenPiece : p1TakenPieces) {
            try {
                final BufferedImage image = ImageIO.read(new File("imgs/"
                        + takenPiece.getPieceAlliance().toString().substring(0, 1) + "" + takenPiece.toString()
                        + ".gif"));
